        ...
        for _ in range(attempts):
            with self._client.pipeline() as pipe:
                try:
                    ...
                    pipe.multi()
                    pipe.set(key, json.dumps(value), ex=ttl_value)
                    ...
                    if ttl_value:
                        meta["expires_at"] = (datetime.utcnow() + timedelta(seconds=ttl_value)).isoformat()
                    pipe.set(f"{key}:meta", json.dumps(meta), ex=ttl_value)

                    pipe.execute()
                    return True
                except redis.WatchError:
                    continue
        raise RuntimeError(f"Failed to append to context key '{key}' after {attempts} attempts.")
...
def delete_context_keys(keys: Iterable[str]) -> Dict[str, bool]:
    """Delete one or more context entries by key."""
    store = get_context()
    ...
    for key in keys:
        ...
        if not normalized:
            continue
        result[normalized] = store.delete(normalized)
    return result
